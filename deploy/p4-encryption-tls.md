...
This is the Phase P4 runbook for the remote MCP daemon on devbox (Model 1:
...
Rather than re-encrypting the system Postgres (which also holds the operator's live
...
| LUKS key         | **1Password → Personal → "Persistor LUKS key (devbox)"** (document). NO on-disk copy. |
...
| Daemon           | systemd `persistor-server.service` (User=user, RW only to the notes dir); NOT boot-enabled |
...
stolen powered-off disk cannot be unlocked — opening the volume requires the operator's
...
# https://devbox.example.ts.net  (tailnet only — NOT `funnel`)
```
