# Remote-primary deployment (devbox)

The devbox deployment where the **encrypted remote store is the operator's primary
...
- **Source of truth:** the git repo `/home/user/code/demo` (+ the Claude
  memory dir), GitHub-backed. `.md` files are authoritative; the DB is a
  rebuildable index.
- **Shared index:** Postgres `:5434`, tenant `c6dc079c-…` (= uuidv5 of the operator's
  Stytch identity), data dir on the LUKS volume `/mnt/persistor-enc`.
- **devbox stdio `persistor`** (`~/.persistor/env`): `DATABASE_URL` → `:5434`,
...
The LUKS volume is locked at boot, so **the operator has no memory on devbox until
you unlock it**:
...
Then restart Claude Code on devbox. (Note: `:5432` is frozen at cutover time —
...
- Remote writes go into the git repo (unencrypted on disk, but GitHub-private —
  the operator's established posture). The encrypted `:5434` DB encrypts the index/note
