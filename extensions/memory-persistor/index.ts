...
const memoryPersistorPlugin = {
  ...
  description:
    "Unified memory search across workspace files and Persistor knowledge graph",
  ...
  register(api: OpenClawPluginApi) {
    ...
    persistorClient.checkHealth().then((ok) => {
      console.log(ok
    ...
    api.registerCli(
      ({ program }) => {
        ...
        const kg = program
          .command("memory-kg")
          .description("Persistor knowledge graph memory");
