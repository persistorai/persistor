...
`
	n := index.ParseNote("scout", "memory/x.md", content, false)
	if n.ID != "my-note" {
...
func TestParseNote_DerivedDefaults(t *testing.T) {
	...
	n := index.ParseNote("scout", "memory/daily/2026/06/2026-06-17.md", content, false)
	want := "scout:memory-daily-2026-06-2026-06-17"
	if n.ID != want {
...
func TestParseNote_CoreTier(t *testing.T) {
	n := index.ParseNote("scout", "identity/SOUL.md", "# Soul\n\nWho I am.\n", true)
	if n.Tier != "core" {
...
func TestParseNote_NamespacePreventsCollision(t *testing.T) {
	a := index.ParseNote("scout", "MEMORY.md", "# Mem\n\nA\n", true)
...
func TestParseNote_NoFrontmatterButLeadingDashes(t *testing.T) {
	...
	content := "Some intro.\n\n---\n\nMore text.\n"
	n := index.ParseNote("scout", "x.md", content, false)
