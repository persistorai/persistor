...
func TestValidateNamespace(t *testing.T) {
	valid := []string{"", "default", "demo", "work-2026", "a.b_c-d", "n0"}
	for _, ns := range valid {
		if err := index.ValidateNamespace(ns); err != nil {
			t.Errorf("ValidateNamespace(%q) = %v, want nil", ns, err)
		}
	}
	invalid := []string{"the operator", "sc out", "demo:sub", "-lead", ".lead", "ns/slash", "ns|pipe", strings.Repeat("a", 129)}
...
func TestValidateNoteID_Exported(t *testing.T) {
	if err := index.ValidateNoteID("demo:memory-foo"); err != nil {
