...
func TestApplyPlan_WritesProseNotes(t *testing.T) {
	...
	plan := &index.Plan{Notes: []index.PlanNote{
		{
			...
			Title: "Weight 2026", Supersedes: "demo:weight",
			Body: "# Weight 2026\n\nWeighs 230 pounds now.",
		},
		{
			ID: "demo:core-summary", Path: "core/summary.md", Tier: "core",
	...
	content := string(raw)
	for _, want := range []string{"kind: fact", "tier: tail", "supersedes: demo:weight", "Weighs 230 pounds now."} {
		mustContain(t, content, want)
	}

	// Round-trip: ParseNote reads back the tier + supersedes the plan wrote.
	n := index.ParseNote("demo", "weight-2026.md", content, false)
	if n.Tier != "tail" || n.Supersedes != "demo:weight" {
		t.Errorf("parsed note = %+v, want tier=tail supersedes=demo:weight", n)
	}
...
func TestApplyPlan_RejectsBadPlans(t *testing.T) {
	cases := []struct {
		...
		{"id with slash", index.PlanNote{Path: "note.md", ID: "demo/big-jerry", Body: "x"}},
	}
...
func TestConsolidate_RoundTrip(t *testing.T) {
	...
	seed := &index.Plan{Notes: []index.PlanNote{
		{ID: "demo:weight", Path: "weight.md", Body: "# Weight\n\nWeighs 250 pounds at the desk."},
	}}
	...
	correction := &index.Plan{Notes: []index.PlanNote{
		{ID: "demo:weight-2026", Path: "weight-2026.md", Supersedes: "demo:weight",
			Body: "# Weight 2026\n\nWeighs 230 pounds now."},
	}}
	...
	if hasHit(hits, "demo:weight") {
		t.Errorf("stale note retrievable by default: %v", ids(hits))
	}
	if !hasHit(hits, "demo:weight-2026") {
