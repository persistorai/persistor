...
func TestIndexer_IndexPath(t *testing.T) {
	...
	scoutDir := t.TempDir()
	writeFile(t, scoutDir, "memory/atomic/a.md", "---\nid: note-a\n---\n\nAlpha fact.\n")
	roots := []index.Root{{Name: "scout", Dir: scoutDir, Includes: []string{"memory"}}}

	// Targeted index of a single written file, no full walk of the root.
	abs := filepath.Join(scoutDir, "memory", "atomic", "a.md")
	...
	assertPaths(t, rep.Paths, []string{"scout/memory/atomic/a.md"})
	...
	writeFile(t, scoutDir, "memory/atomic/b.md", "---\nid: note-b\nsupersedes: note-a\n---\n\nAlpha, corrected.\n")
	abs = filepath.Join(scoutDir, "memory", "atomic", "b.md")
	rep, err = ix.IndexPath(ctx, tenantID, roots, abs)
	if err != nil {
