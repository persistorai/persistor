...
func TestListNotes_PaginationAndNamespace(t *testing.T) {
	...
	seedNote(t, store, tenant, "demo", "demo:a", "alpha")
	seedNote(t, store, tenant, "demo", "demo:b", "bravo")
	seedNote(t, store, tenant, "demo", "demo:c", "charlie")
	...
	if all[0].ID != "claude:a" || all[4].ID != "demo:c" {
	...
	// Namespace filter.
	demo, err := store.ListNotes(ctx, tenant, index.ListOpts{Namespace: "demo"})
	if err != nil {
		t.Fatalf("list demo: %v", err)
	}
	if len(demo) != 3 {
		t.Fatalf("demo listed %d, want 3", len(demo))
	}
	for _, n := range demo {
		if n.Namespace != "demo" {
			t.Fatalf("got namespace %s in a demo-only list", n.Namespace)
		}
	}

	// Pagination within the namespace.
	p1, err := store.ListNotes(ctx, tenant, index.ListOpts{Namespace: "demo", Limit: 2, Offset: 0})
	if err != nil {
		t.Fatalf("page 1: %v", err)
	}
	p2, err := store.ListNotes(ctx, tenant, index.ListOpts{Namespace: "demo", Limit: 2, Offset: 2})
	if err != nil {
	...
	if p1[0].ID != "demo:a" || p1[1].ID != "demo:b" || p2[0].ID != "demo:c" {
...
func TestNamespaces_Counts(t *testing.T) {
	...
	seedNote(t, store, tenant, "demo", "demo:a", "x")
	seedNote(t, store, tenant, "demo", "demo:b", "x")
	...
	if nss[1].Namespace != "demo" || nss[1].Count != 2 {
		t.Fatalf("nss[1] = %+v, want demo/2", nss[1])
	}
}
