...
func TestWriteLimiter(t *testing.T) {
	...
	if !mcpengine.NewWriteLimiter(0, 0).Allow("t") {
		t.Error("disabled limiter should allow")
	}
	var nilLimiter *mcpengine.WriteLimiter
	if !nilLimiter.Allow("t") {
		t.Error("nil limiter should allow")
	}

	// Burst 2: the first two writes pass, the third is denied (refill is too slow
	// to matter within the test).
	l := mcpengine.NewWriteLimiter(0.001, 2)
