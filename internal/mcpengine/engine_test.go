...
func newTestEngine(t *testing.T) *mcpengine.Engine {
	...
	roots := []index.Root{{Name: "demo", Dir: dir, CorePaths: []string{"identity/SOUL.md"}}}
...
func TestEngine_RoundTrip(t *testing.T) {
	...
	wr, err := e.Write(ctx, &mcpengine.WriteInput{
		Path: "aurora-v2.md", ID: "demo:aurora-v2", Supersedes: "demo:memory-daily-aurora",
...
func TestEngine_WriteRejectsSelfSupersede(t *testing.T) {
	...
	// The note at notes/widget.md resolves to id "demo:widget" (the test engine's
	// notesDir is <dir>/memory/atomic under root "demo"; derive accordingly).
	first, err := e.Write(ctx, &mcpengine.WriteInput{Path: "widget.md", Body: "# Widget\n\nOriginal."})
	if err != nil {
		t.Fatalf("first write: %v", err)
	}
	selfID := "demo:" + "memory-atomic-widget"
