...
func newTestEngine(t *testing.T) (*mcpengine.Engine, *index.Store) {
	...
	seed(t, store, tenantID, &index.PGNoteInput{
		ID: "demo:soul", Tier: "core", Title: "Soul",
		Body: "We are the Northwind expedition crew.", Surface: "test",
	})
	seed(t, store, tenantID, &index.PGNoteInput{
		ID: "demo:memory-daily-aurora", Title: "Aurora Protocol",
...
func TestEngine_RoundTrip(t *testing.T) {
	...
	if !hasResult(sr.Results, "demo:memory-daily-aurora") {
		t.Fatalf("search missed aurora: %+v", sr.Results)
	}

	// get: returns the full body and the current version.
	gr, err := e.Get(ctx, mcpengine.GetInput{ID: "demo:memory-daily-aurora"})
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	...
	wr, err := e.Write(ctx, &mcpengine.WriteInput{
		ID: "demo:aurora-v2", Supersedes: "demo:memory-daily-aurora",
		Body: "# Aurora Protocol v2\n\nUpdated polar storm protocol.",
	})
	if err != nil {
		t.Fatalf("write: %v", err)
	}
	if wr.ID != "demo:aurora-v2" || wr.Op != "create" || wr.Version != 1 || wr.Superseded != 1 {
		t.Fatalf("write = %+v, want id=demo:aurora-v2 op=create version=1 superseded=1", wr)
	}
	...
	if !hasResult(sr.Results, "demo:aurora-v2") {
...
func TestEngine_WriteRejectsMissingSupersedeTarget(t *testing.T) {
	...
	_, err := e.Write(ctx, &mcpengine.WriteInput{
		ID: "ghost", Supersedes: "demo:does-not-exist", Body: "# Ghost\n\nbody",
	})
	if err == nil {
